      ...
      <div className="modal-content" onClick={e => e.stopPropagation()}>
        ...
        <div className="alternatives-section">
          ...
          {alternatives.length === 0 ? (
            <p style={{ color: 'var(--text-secondary)' }}>No direct alternatives found matching the capabilities and context window.</p>
          ) : (
            alternatives.map((alt) => (
              <div key={alt.model.model_id} className="alt-card">
                <div>
                  <h4 style={{ fontSize: '1.1rem', color: 'var(--text-primary)' }}>{alt.model.name}</h4>
